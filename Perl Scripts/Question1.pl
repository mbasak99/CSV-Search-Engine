...
my @butter_per_year_sum = 0;
my @cheese_per_year_sum = 0;
...
$output_file = "Q1".$provinces[$answer_province - 1].".txt";
...
while (my $text_file_record = <$file_in_use>){
	...
	#	Taking out only the relevant info into an array (Butter and Cheese for the selected province)
	if(($master_fields[1] eq $provinces[$answer_province - 1]) && ($master_fields[4] =~ m/^(\d*.24)$/ || $master_fields[4] =~ m/^(\d*.25)$/) ){
		$sorted_record_count++;
		...
		if($master_fields[4] =~ m/^(\d*.24)$/){
			$butter_per_year_sum[$increment] +=  $master_fields[5];
		}
		if($master_fields[4] =~ m/^(\d*.25)$/){
			$cheese_per_year_sum[$increment] +=  $master_fields[5];
		}
	}
}
...
for my $i (0..$increment){
			$butter_per_year_sum[$i] = $butter_per_year_sum[$i] / 12;
}

for my $i (0..$increment){
			$cheese_per_year_sum[$i] = $cheese_per_year_sum[$i] / 12;
}
...
for my $i (2000..2016){
			...
			$output_record_count++;
			$newline = "Butter,".$i.",".$butter_per_year_sum[$i - 2000]."\n";
			$output_records[$output_record_count] = $newline;

			$output_record_count++;
			$newline = "Cheese,".$i.",".$cheese_per_year_sum[$i - 2000]."\n";
			$output_records[$output_record_count] = $newline;
}
...
$pdffilename = "Q1".$provinces[$answer_province - 1].".pdf";
...
$R->run(qq`graph_title <- substitute(paste("Comparing Butter and Cheese for ", ${edited_provinces[$answer_province - 1]}))`);
